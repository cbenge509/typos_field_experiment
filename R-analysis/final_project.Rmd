...
model_ucb_treatment_pretreat <- treatment_ucb[, lm(Intelligence ~ Treatment + length + length + Gender + English + Race + isUS + Degree + age_bins + ReadSocialMedia + WriteSocialMedia + Intelligence.pretreat + Writing.pretreat + Interest.pretreat + Effective.pretreat)]
...
```{r hetero age}
model_ucb_treatment_gender_h <- treatment_ucb[, lm(Intelligence ~ Treatment + length + length + Gender + English + Race + isUS + Degree + age_bins + ReadSocialMedia + WriteSocialMedia + Intelligence.pretreat + Writing.pretreat + Interest.pretreat + Effective.pretreat + age_bins:Treatment)]
coeftest(model_ucb_treatment_gender_h, vcov. = vcovHC)
```
...
model_ucb_treatment_mistakes <- treatment_ucb[, lm(Intelligence ~ Treatment + length + length + Gender + English + Race + isUS + Degree +age_bins + ReadSocialMedia + WriteSocialMedia + Intelligence.pretreat + Writing.pretreat + Interest.pretreat + Effective.pretreat + Knowledge + CorrectlyNoticedErrors)]
coeftest(model_ucb_treatment_mistakes, vcov. = vcovHC)
```
Even for controlling for this, there's no effect! So it doesn't even matter what they noticed. 

Analysis section order in the paper so far:

1. EDA...download latest and fully clean the data
2. Placebo test: mech turk vs berkeley data -- restrict to the berkeley data
3. base model with no covariates added
4. add model with properties of the post 
5. add all of the demographics
6. add baseline variables for pre-treatment placebo responses -- visualize this
7. add effect of seeing other social media post first
8. Is treatment effect stronger in male or female? Strong in older individuals than younger?
9. whether they actually noticed any errors?

Two experiments together: Mech Turk vs Berkeley
